...
export function useTableState(props) {
  ...
  // Header actions: prefer page-level prop (from ListUsers::getHeaderActions()),
  // fallback to table schema (backward compat)
  const headerActions = computed(() =>
    pageProps.value.headerActions?.length
      ? pageProps.value.headerActions
      : (tableConfig.value?.headerActions || [])
  )
